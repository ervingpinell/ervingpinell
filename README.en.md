...
Receptionist by obligation, developer by profession, gamer by decision.

I work from Costa Rica, mostly with **Laravel, JavaScript, PHP and PostgreSQL**.

Before I started programming I was a receptionist in tourism, dealing with guests
in Spanish and English every day.
...
Live at: **[greenvacationscr.com](https://greenvacationscr.com)** ·
**[lutantravelcr.com](https://lutantravelcr.com)**

An ERP and booking engine for tour operators in Costa Rica. It isn't a booking
site with an admin panel behind it: it covers a tour company's whole operation,
...
French, German and Portuguese) with `hreflang`, and its admin panel behind it.
...
| **Sales channels** | Viator and GetYourGuide connected supplier side: availability, booking, amendment and cancellation, with outbound notices to the agency. |
...
| **E-invoicing** | Filing with the Costa Rican tax authority (v4.4): invoice, ticket, credit note, purchase invoice and receiver message. XAdES-EPES signing with the client's certificate, validation against the official schemas, and control of the document key and sequence. |
...
| **HR and payroll** | Payroll with Costa Rican social charges, job openings, candidates, interviews, absences and severance. ZETKO integration coming soon. |
...
- **Multi-company, never a shared database.** Every client has its own database.
  The server can be shared, the database never.
- **The electronic signature, written by hand.** XAdES-EPES in PHP, no library in
  between: if the XML comes out badly signed, the tax authority rejects it and
  there is no invoice. It's validated against the official schemas before being
  sent, with one detail that took a while to find: an unsigned XML reports a
  false "missing node", so the validator inserts a stub signature to avoid
  fooling itself.
...
for the phone: the client books from their handset while they're travelling.

Plain HTML, CSS and JavaScript, no build step, no framework, about 1,200 lines in
total, served as static files. Switching languages is one attribute per string,
and booking ends in WhatsApp with the message already written, which is where the
business actually replies.

It loads fast even on hotel wifi, which was the point.
...
- **No browser dependencies.** HTML, CSS and JavaScript served as they are: no
  build, no framework, no `node_modules`. The page opens before a framework would
